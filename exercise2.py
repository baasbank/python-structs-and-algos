...
# Implement the __mul__ method for the Vector class of Section 2.3.3, so that the expression u * v
# returns a scalar that represents the dot product of the vectors, that is, ∑di=1 ui · vi .
...
# Another convenient form for creating a new vector would be to send the constructor a parameter that is
# some iterable type representing a sequence of numbers, and to create a vector with dimension equal to the
# length of that sequence and coordinates equal to the sequence values.
# For example, Vector([4, 7, 5]) would produce a three-dimensional vector with coordinates <4, 7, 5>.
# Modify the constructor so that either of these forms is acceptable;
# that is, if a single integer is sent, it produces a vector of that dimension with all zeros,
